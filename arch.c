...
static void startAction(actionQueue_t *q, action_t * pAction)
{
	...
	case	CACT_F000:
		q->flag = (1 << 3);		
		if(pAction->func != 0)((paction_t_0)(pAction->func))();
		break;
	
	/** 两个函数指针, 无参数 **/
	case	CACT_F0F0:
		q->flag = (1 << 2);		
		if(pAction->actionPara != 0)((paction_t_0)(pAction->actionPara))();
		if(pAction->func != 0)((paction_t_0)(pAction->func))();
		break;
	...
	case	CACT_DFLT:
		q->flag = (1 << 1);
		if(pAction->func != 0)((pfunc_t)(pAction->func))((void *)pAction->actionPara);
		break;
...
void actProcess(actionQueue_t * q)
{
	...
	if(actionQueueOut(q, &action) == TRUE) {
		...
		if(1) {
			q->flag = 0;
			SetTimer_irq(q->timer, CTIMER_100MS, q->overMsgType);
		}
	}
}
